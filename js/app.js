import { calculateRankings, validateHistory, validatePlayers } from "./rankings.js";
...
export function updateLastUpdatedLabel(matches, element) {
  ...
  if (!latest?.updatedAt || !element) return;
  element.textContent = `Classifica aggiornata il ${formatUpdatedAt(latest.updatedAt)}`;
}
...
async function main() {
  ...
  updateLastUpdatedLabel(matches, document.querySelector(".aggiornamento"));
  const rankings = calculateRankings(players, matches);
  for (const stat of sections) renderRanking(stat, rankings[stat]);
}
